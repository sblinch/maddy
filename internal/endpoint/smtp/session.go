...
func (s *Session) logDeliveryAttempt(err error) {
	mod := "internal"
	...
	msgMetaID := ""
	if s.msgMeta != nil {
		msgMetaID = s.msgMeta.ID
	}

	remoteAddr, _, _ := net.SplitHostPort(s.connState.RemoteAddr.String())
	s.log.Msg("delivery",
		"id", msgMetaID,
		"from", s.mailFrom,
		"rcpt", s.rcptTo,
		"remote", remoteAddr,
		"helo", s.connState.Hostname,
		"module", mod,
